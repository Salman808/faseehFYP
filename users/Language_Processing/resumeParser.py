...
class Parse():
    ...
    def getQualification(self,inputString,infoDict,D1,D2):
        ...
        file_addr = os.path.join(BASE_DIR, 'users', 'Language_Processing',"nameofinstitutes.txt")
        nameofinstitutes=open(file_addr,'r').read().lower()#open file which contains keywords like institutes,university usually  fond in institute names
    ...
    def Qualification(self,inputString,infoDict,debug=False):
        ...
        #  degre.append('ca')
        self.getQualification(self.inputString, infoDict, 'bs', 'BS (Computer Science)')
        if infoDict['%sline' % 'bs']:
            degre.append('BSCS')
        self.getQualification(self.inputString, infoDict, 'Inter', 'intermediate in computer')
        if infoDict['%sline' % 'Inter']:
            degre.append('ICS')
        self.getQualification(self.inputString, infoDict, 'BSCS Hons', 'BS (Computer Science)')
        if infoDict['%sline' % 'BSCS Hons']:
            degre.append('BSCS')
        self.getQualification(self.inputString, infoDict, 'bsc', 'SE (Software Engineering)')
        if infoDict['%sline' % 'bsc']:
            degre.append('BSSE')
